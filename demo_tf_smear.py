...
slices = [tf.slice(srnn_output, [0,start], [batch_size, feature_width]) for start in [0, feature_width]]

smodel_logitss = [tf.matmul(slice, model_fc_w) + model_fc_b for slice in slices]
#smodel_logits1 = tf.matmul(slice1, model_fc_w) + model_fc_b
#slice1 = slices[0]
smodel_logits1 = smodel_logitss[0]
smodel_predict1 = tf.nn.softmax(smodel_logits1, name="smodel_predict1")

#slice2 = slices[1]
#smodel_logits2 = tf.matmul(slice2, model_fc_w) + model_fc_b
smodel_logits2 = smodel_logitss[1]
smodel_predict2 = tf.nn.softmax(smodel_logits2, name="smodel_predict2")
